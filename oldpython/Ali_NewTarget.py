...
database_name='E:/work/data/dress0615.db'
...
#更新订单数量
def updateorders(database_name):
...
def DelNotNeedRecord(database):
    
    con = sqlite3.connect(database)
    cur = con.cursor()
    cur.execute('DELETE FROM tab_dress where record=3 and order1<order0+10 and order2<order1+10')
    con.commit()
    con.close()

def IsUrlExist(url, oldlist):
    index = 0
...
oldlist = []
#1. 更新订单数据
updateorders(database_name)

#2. 删除淘汰数据 record=3 and order1<order0+10 and order2<order1+10
record_num=3
adden=10
#DelNotNeedRecord(database_name)

#4. 扫描新的目标
new_list = []
key_list=[]

addr = "http://www.aliexpress.com/category/200003482/dresses.html?site=glo&SortType=create_desc&shipCountry=us&g=n&needQuery=n"
