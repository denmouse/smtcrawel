...
def get_order_info(strs):
    ...
    lstproducts = strs.split("info infoprice")
...
def getNewList(url,key):
    ...
    #order_list写入数据库
    con = sqlite3.connect('E:/targetprod.db')
    cur = con.cursor()
...
con = sqlite3.connect('E:/targetprod_0429.db')
cur = con.cursor()
cur.execute('SELECT * FROM tab_dress')
#print cur.fetchall()
...
for key in key_list:
    print key
    if 'jumpsuit' in key:
        newaddress = read_newaddress_cat("jumpsuit")
    else:
